...
const FEATURES: Array<[string, string, string]> = [
  ...
  ["Render Quality Tiers (premium)", "✅ Working", "`_shared/render-quality.ts` is the single source of truth for Creatomate render options. Tier defaults to `premium`: shorts render at full scale (no 0.75 downscale) at 60fps, recaps at full 1080p/30fps (30fps keeps 60s+ recaps inside the poll window). Options are spread LAST into the /v2/renders body so they override composition-level frame_rate, and every render sets `snapshot_time` on the hero-temperature frame for a stronger auto thumbnail. Shorts typography upgraded: hero temp 180→232px with heat-aware fill (`tempAccentColor`: red/orange hot → blue cold), city 88→96px, condition 38→44px bold, metric values 44→52px, metric labels 26px at higher contrast, premium drop shadow. Env overrides: RENDER_QUALITY_TIER (premium|standard|eco), SHORTS_FRAME_RATE, RECAP_FRAME_RATE. Log line: `[render_quality] {caller} kind=… tier=… fps=… render_scale=…`."],
];
...
- **Phase 10C — Date-stamped winner template (\`buildDateStampedTitle\` in \`_shared/title-builder.ts\`)**: \`{City} Weather | {Weekday}, {Month} {Day}\` (ET-anchored). Reproduces the 159-view winning hook pattern. Fallback pattern only as of Phase 14A. Validated by \`title-builder.test.ts\` against \`validatePostBundle\` for all fixture cities/temps/conditions.
- **Phase 14A — Urgency & Specificity title formula (\`buildFormulaTitleBody\` in \`_shared/title-builder.ts\`)**: \`{City}: {Specific Detail} - {Hook/Value Prop}\`, body capped at \`TITLE_BODY_BUDGET = 52\` chars so \`[8 AM] \` + body stays under 60 chars for mobile SERPs. Trigger precedence (first match wins): feels-like ≥100°F → \`extreme_heat\` ("Feels Like 103°F - Stay Cool Tips") · temp ≥95°F → \`extreme_heat\` ("95°F Today - Heat Advisory Details") · storm condition or rain ≥70% → \`storm\` ("Severe Storms - Timing & Impacts") · rain condition or rain >50% → \`rain\` ("Rain 60% Likely - Hour-By-Hour Timing") · temp ≤35°F → \`extreme_cold\` ("32°F Freeze Risk - What To Protect") · snow → \`snow\` · |tomorrowHigh − temp| ≥8°F → \`pattern_change\` ("92°F Now - Relief At 78°F Tomorrow" / "Warm-Up Ahead - 84°F By Tomorrow") · ET weekday Thu/Fri → \`weekend\` ("75°F & Sunny - Weekend Outlook") · temp ≥88°F → \`heat\` · temp ≤45°F → \`cold\` · pleasant 68–84°F clear/partly → \`pleasant\` ("Sunny & 75°F - 7-Day Outlook"). When no trigger fires (\`trigger: "none"\`) selection falls back to Phase 10C date-stamp (~70%) / weather-variety pool (~30%). Context (\`feelsLike\`, \`tomorrowHigh\`, \`tomorrowLow\`, \`tomorrowCondition\`) is passed as the optional 7th arg of \`generateSkyBriefTitle\` by \`process-scheduled-posts\` and \`daily-weather-post\`. All templates avoid \`BANNED_FRAGMENTS_STRICT\` and \`BANNED_LOCATION_PROXIES\`; \`[title_debug] formula trigger=…\` is logged on every build. Covered by 3 additional tests in \`title-builder.test.ts\` (length + prefix + validation, trigger precedence, banned-fragment sweep).`;
...
| Phase 7B — Creatomate renders 100% failing because \`BROADCAST_BG_MUSIC_URL\` secret pointed at a deleted Pixabay asset (\`Action-Trailer-Logo-3-149997.mp3\`) returning 404 | Root cause: Creatomate fetched the bg-music URL during render init and aborted the whole job when the asset 404'd; voiceover + chime were fine but the dead music link nuked every render in the pipeline. Fix in \`process-scheduled-posts/index.ts\`: added a known-dead-asset blocklist (\`KNOWN_DEAD_BG_MUSIC_URLS\`) that suppresses the bg-music layer at request build time and emits \`[bg-music] BROADCAST_BG_MUSIC_URL matches known-dead asset — disabling bg music layer\`. Voiceover + intro chime remain so audio is not silent. Verified end-to-end: post \`f0c81956\` rendered in ~22s (\`planned → rendering → succeeded\`, 3.2 MB, 12s) on the first attempt with zero new \`render.creatomate.*_error\` rows. Replacing the secret with a working royalty-free loopable URL is deferred until after Phase 8 |
| Phase 8B — Location-proxy validator false-positives blocked legitimate weather narration ("Heads up for rain", "clear skies tonight", "storms coming up") because \`BANNED_FRAGMENTS_STRICT\` matched as a global substring across title + description | Root cause: \`BANNED_FRAGMENTS_STRICT\` in \`supabase/functions/_shared/text-sanitizer.ts\` did a word-boundary substring match over the entire title + description, so common weather phrases like "Heads up — storms tonight" were blocked even when the city was correct (3 of 8 production blocks in 7 days were false positives). Fix: moved "Coming Up", "Clear Skies", "Heads Up" from \`BANNED_FRAGMENTS_STRICT\` → \`BANNED_LOCATION_PROXIES\`, which only blocks the phrase when it appears inside a location slot (\`(in|for|over|across|around|near)\\s+<Phrase>\`). \`BANNED_FRAGMENTS_STRICT\` now only contains the two strong-signal hallucinations: "Weather Update" (4/8 production hits, pure proxy) and "Not Need" (garbled artifact). Prompt hardening in \`generate-caption/index.ts\` \`locationGuard\`: added "Heads Up" to the forbidden-as-location list and added explicit ALLOWED-vs-FORBIDDEN narration examples ("heads up for rain tonight" vs "in Heads Up", "clear skies tonight" vs "across Clear Skies", "storms coming up this evening" vs "over Coming Up"). Added 11-test coverage in \`text-sanitizer.test.ts\` (one block + one allow per phrase, plus a mixed-narration regression test) and updated the existing title-builder strict-block test to use the new slot-context form. All 16 tests pass; \`process-scheduled-posts\`, \`daily-weather-post\`, \`generate-caption\` redeployed |
| Phase 12B — Weekly + Monthly Recap auth migration: recap functions still read the legacy \`weather_settings.youtube_refresh_token\` (revoked when users reconnected via the multi-city flow), so every cron fired but failed with \`post_history.error_message = "No YouTube token for weekly recap"\` | Replaced the inline \`getYouTubeToken(svc, userId)\` in both \`create-weekly-recap\` and \`create-monthly-recap\` with new shared helper \`_shared/recap-youtube-token.ts\` (\`getRecapYouTubeToken\`, \`listYouTubeConnectedUserIds\`, \`resolveCityIdByName\`). The helper resolves the recap's city by name → \`cities.id\`, then delegates to the same \`YouTubeAdapter.getValidToken\` used by the Shorts pipeline so per-city \`social_accounts\` rows are preferred with automatic legacy \`weather_settings\` fallback. Both functions' user-discovery query now unions \`social_accounts (platform=youtube)\` with legacy \`weather_settings\` so users connected only through the new flow are eligible. On failure the recap inserts a typed \`[AUTH_EXPIRED:youtube]\` / \`[ROUTING_VIOLATION]\` error_message into \`post_history\` AND a matching \`notifications\` row (\`type='error'\`, "Reconnect YouTube for {city}" / "Assign a YouTube channel to {city} in Settings") so the user sees the failure in the in-app bell instead of needing SQL. Daily recaps intentionally not implemented (Essential-tier credit budget: 1810/2000). Cron schedules unchanged (weekly Sundays, monthly month-end). No schema changes |
 | Phase 12CB — five recap fixes: (1) Orlando weekly recap published with **no voiceover** because Creatomate silently clamped \`volume: 2.0\` (and monthly's \`2.5\`) — values >1 are dropped when expressed as a raw number; (2) Gainesville **never got a recap** because the dispatcher looped per-user and routed to \`posts[0].city\`, which Orlando won by 17s every week; (3) Orlando afternoon Short blocked by \`banned_fragment\` on the legitimate narration phrase "Weather Update"; (4) voice synth failures were invisible — recap rendered silent with no notification; (5) no pre-flight on the voice signed URL before submitting to Creatomate | Fix #1: swapped voice \`volume\` to Creatomate's spec-compliant percentage-string format (\`"200%"\` weekly, \`"250%"\` monthly) preserving the boost over the \`0.15\` music bed. Fix #2: added \`listUserRecapCities(svc,userId)\` to \`_shared/recap-youtube-token.ts\` (joins \`user_cities\` → \`cities\`, falls back to legacy \`weather_settings.city\`) and rewrote both recap dispatchers to expand each connected user into one \`(user, city)\` job per connected city — Gainesville now generates its own recap alongside Orlando. Manual \`city\`-body invocations still short-circuit to a single city. Fix #3: moved \`"Weather Update"\` from \`BANNED_FRAGMENTS_STRICT\` → \`BANNED_LOCATION_PROXIES\` in \`_shared/text-sanitizer.ts\` so it only blocks the phrase inside an \`(in|for|over|across|around|near) Weather Update\` slot; legitimate narration like "Weather Update for the day ahead" is allowed. Added two new \`text-sanitizer.test.ts\` cases (block + allow); all 12 tests pass. Fix #4: after \`synthesizeRecapVoice\` returns null, both recap \`runForUser\` functions check \`weather_settings.enable_voiceover\` and, when true, insert a \`notifications\` row (\`type='warning'\`, "Voiceover synthesis failed for {city}") and a \`system_logs\` row (\`type='render.voice.fallback'\`) so silent failures stop slipping past. Fix #5: HEAD-probe the voice signed URL before passing it to \`stitchSlideshow\`; on non-2xx, non-audio content-type, or sub-1KB length, drop the voice track, log \`render.voice.preflight_failed\` to \`system_logs\`, and render silent rather than letting Creatomate silently drop the bad asset. All edits scoped to \`create-weekly-recap/index.ts\`, \`create-monthly-recap/index.ts\`, \`_shared/recap-youtube-token.ts\`, \`_shared/text-sanitizer.ts\`, and \`_shared/text-sanitizer.test.ts\`. No schema changes, no cron-schedule changes
 | Phase 12CC-B — May monthly recaps never ran AND a future Miami expansion would have generated unwanted recaps: (1) the \`create-monthly-recap-month-end\` cron job was rescheduled without the \`x-cron-secret\` header, so every month-end POST was rejected by \`requireCronOrUser\` at the auth gate — silent 401, no \`post_history\` row, no \`system_logs\` evidence; (2) \`listUserRecapCities\` returned **every** \`user_cities\` row, so once Miami was attached to the user it would have been swept into recap dispatch despite having no channel mapping; (3) dispatcher had no audit trail — when a recap dropped (auth, missing city, etc.) there was no system_logs evidence | Fix #1: re-issued the monthly cron via \`cron.unschedule\` + \`cron.schedule\` to mirror the weekly cron exactly — loads \`vault.decrypted_secrets.cron_secret\` into a \`DO\` block and sends \`x-cron-secret\` + \`Content-Type\` + \`apikey\` headers with \`timeout_milliseconds := 60000\`. Schedule unchanged (\`59 23 28-31 * *\`). Fix #2: \`listUserRecapCities\` now delegates to new \`listUserRecapCitiesDetailed(svc,userId)\` in \`_shared/recap-youtube-token.ts\`, which joins \`user_cities\` → \`cities\` and gates each city on \`automations.enabled = true\` for that \`(user_id, city_id)\` — cities without an enabled automation row are filtered out with \`reason='automation_disabled_or_missing'\`. Legacy \`weather_settings.city\` fallback only fires when the user has zero \`user_cities\` rows (single-city users untouched). Returns \`{cities, skipped[], source}\`. Fix #3: both \`create-weekly-recap\` and \`create-monthly-recap\` dispatcher loops use \`listUserRecapCitiesDetailed\` and after expansion insert a \`system_logs\` row \`type='recap.dispatch'\` with \`{recap_type, user_count, candidate_count, duration_ms, trigger, per_user:[{user_id, cities[], skipped[], source}]}\` so every recap run leaves a complete audit trail of which cities were kept, which were skipped, and why. Self-healing recap queue (\`recap_runs\` table) intentionally deferred to Phase 13 |
 | Phase 12CC-D — both May monthly recaps rendered as **infographic-only fallbacks with no voiceover** because the Creatomate request was rejected at submit time with \`"Audio.volume: Expected a value between 0 and 200"\` — the monthly recap voice track was using \`volume: "250%"\`, which exceeds Creatomate's documented maximum of \`"200%"\`. Weekly recaps (already at \`"200%"\`) rendered fine, confirming the cap. The render failure triggered the infographic fallback path, which doesn't carry voice | Lowered \`volume: "250%"\` → \`"200%"\` in \`create-monthly-recap/index.ts\` (the voice element appended to the Creatomate composition), matching the working weekly value. Still a 2× boost over the \`0.15\` music bed. No schema changes, no cron-schedule changes, no other edits |`;

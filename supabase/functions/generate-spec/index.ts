...
| Gainesville daily renders were plain ~90% of the time (condition scenes rarely matched + no city-specific safe fallback) | Implemented Cinematic Preset System (\`_shared/cinematic-presets.ts\`). Gainesville 4-tier fallback chain: (1) history image from post_history if any, (2) matching weather-condition stock scene (condition_scene), (3) city safe scene (Gainesville oak canopy / downtown skyline / prairie storm JPGs in brand-assets/cinematic/), (4) gradient_only last resort. Added \`strict_visuals_gainesville\`, \`smart_cost_strategy\`, \`exclude_fallback_from_learning\` toggles to \`weather_settings\`. All three default \`true\`. Renders never fail — every tier is wrapped in try/catch |
| Visual learning loop reinforcing degraded renders (gradient_only / degraded_fallback) as "winners" because no quality gate existed | Added \`isLearningEligibleRow()\` predicate in \`_shared/cinematic-presets.ts\`. Any row where \`published_visual_source\` is \`gradient_only\` or \`degraded_fallback\` (or visual_metadata.eligibleForLearning = false) is excluded from winning-recipes, style-rotation, compute-winner-stats, and analyze-performance. Toggle \`exclude_fallback_from_learning\` in weather_settings gates this; defaults to true |

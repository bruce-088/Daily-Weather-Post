...
const FEATURES: Array<[string, string, string]> = [
  ...
  ["Social integrations: TikTok, YouTube, X/Twitter, LinkedIn, Instagram", "✅ Working", "OAuth flows; Instagram via Graph API. YouTube auth hardened: access_type=offline + prompt=consent + include_granted_scopes guarantee a long-lived refresh_token; legacy refresh_token action scoped to the row matching the refresh_token actually used (no cross-channel overwrite); adapter performs ONE auto refresh + retry on 401 from upload init; standardized [youtube-auth] logging."],

...
| Weekly + Monthly Recap slides rendered with flat themed gradients only — no real scenery | Added \`buildSlideBackground({ time, duration, slideNum, grad, mode, mediaUrl, condition, logPrefix })\` helper in both \`create-weekly-recap/index.ts\` and \`create-monthly-recap/index.ts\`. Each slide now layers a real background scene **behind** a translucent themed gradient: (1) scene resolution priority is explicit \`mediaUrl\` (e.g. \`p.image_url\` from history, label \`history_image\`) → \`pickConditionScene(condition)\` via the new \`CONDITION_SCENES\` map (hotlinked Unsplash CC0 imagery for \`storm_clouds / rainy_sky / snowy_sky / cloudy_sky / bright_sky\`) → gradient-only; (2) scene element is full-frame \`fit: "cover"\` with a 1.00 → 1.05 \`scale\` animation (image) or looping \`video\` element with the same scale tilt (supported for future Shorts callers, unused by recaps today); (3) themed warm/cool/neutral gradient (\`buildAnimatedGradientBg\`) is then layered on top at \`opacity: "25%"\` so the tint reads without hiding the photo; (4) defensive \`try/catch\` around scene construction — on any throw, helper logs \`[recap|monthly-recap] slide N scene failed, gradient-only\` and emits just the full-opacity gradient so renders **never** fail. Wired into call sites: weekly day slides replace the old image/else gradient branch; monthly week slides pass \`w.dominantCondition\`; Moment-of-the-Month slide passes \`moment.post.image_url\` + \`moment.post.condition\`. Title and outro cards intentionally remain gradient-only. New per-slide logs: \`[recap] slide 2 background=image rainy_sky (https://…)\`, \`[recap] slide 3 background=image history_image (…)\`, \`[monthly-recap] slide 4 background=gradient cool (no scene)\`. No schema, secret, or other-function changes |`;

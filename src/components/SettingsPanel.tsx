...
import { Clock, MapPin, Instagram, Video, RefreshCw, Save, CheckCircle, ExternalLink, Youtube, Sun, Sunset, Moon, Twitter, Linkedin, Unlink, Globe, SkipForward, Lightbulb, CalendarClock, Play, Loader2, MessageSquare } from "lucide-react";
...
export function SettingsPanel({
  ...
  return (
    <div className="space-y-4">
      ...
      {voiceoverVisible && (
        <Card className="border-border/50 bg-card/80 backdrop-blur">
          <Collapsible open={voiceoverOpen} onOpenChange={setVoiceoverOpen}>
            <CollapsibleCardHeader
              open={voiceoverOpen}
              icon={<Lightbulb size={16} className="text-primary" />}
              title="AI Voiceover"
              description="Add a short narrated weather brief to auto-posted videos. Applies to YouTube, TikTok, and Instagram only."
              collapsedHint={
                <Badge variant="outline" className="text-[10px]">
                  {settings.enableVoiceover ? "On" : "Off"}
                </Badge>
              }
            />
            <CollapsibleContent>
